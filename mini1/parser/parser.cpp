...
int main() {
  std::string filename = "./collision_data.csv";
  
  CSV csv = makeCSV(filename);

  std::map<std::string, std::map<int, int>> boroughData; // borough -> {zip_code -> count}
  ...
  std::vector<std::string> boroughs = {"MANHATTAN", "BROOKLYNN", "QUEENS", "BRONX", "STATEN ISLAND"};
  ...
  for (const auto& [borough, zipData] : sortedBoroughData) {
      std::cout << "  \"" << borough << "\"\n";  // Enclose in quotes to see spaces
  }
  std::cout << std::endl;
